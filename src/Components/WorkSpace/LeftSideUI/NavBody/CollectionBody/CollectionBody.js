...
const CollectionBody = () => {
    const { setcolId, collEdit, setchangeAction, workSpaceId, setStatus, changeAction,
        setMsg, setError, tabsList, setTabsList, setCurrentActive } = useContext(DataContext);
        ...
        <>
            <div className="w-full h-full  ">
                <div className="h-full">
                    ...
                    {/* collection */}
                    {loader === true ? (
                        <> {newarr?.map((e) => ('' // <CollectionLoader key={e._id} />
                        ))} </>)
                        : (<><div className="w-full h-[89%]  scrollbar-hide overflow-y-scroll">
                            {newarr?.map((e) => (<div key={e._id} className="border-b">
                                <div className={`w-full h-8 ${e.open === true ? "bg-gray-200" : null}  flex items-center 
                                ...
                                {/* request */}
                                {e.toggle ? (
                                    <div className=" w-full"> {collection?.map((ce) => (
                                        <div key={ce._id}> {e._id === ce.parent ? (
                                            <div className="w-full relative group flex cursor-pointer hover:bg-gray-200 py-1 px-2" >
                                                <div className="flex items-center gap-2 w-full " onClick={() => handleRequest(ce)} >
                                                    <p className={`text-xs text-${getDetails(ce?.details).color}-600 w-[70px] min-w-[70px] flex justify-end`} >
                                                        {getDetails(ce?.details).method}
                                                    </p>
                                                    <p className="text-xs font-normal truncate"> {ce.name} </p>
                                                </div>
                                                ...
                                                {/* moreaction */}
                                                {ce.openRequest ? (<div className="absolute z-50 right-3 top-9">
                                                    {/* <MoreAction {...{ collection: "collection" }} /> */}
                                                </div>) : null}
                                            </div>) : null}
                                        </div>
