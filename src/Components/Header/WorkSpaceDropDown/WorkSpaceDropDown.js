      ...
      >
        <div className="w-full h-full flex flex-col gap-2">
          {Loder === true ? (
            <>
              <div
                className="w-full bg-gray-200 px-4 py-4 rounded-md text-sm font-medium
                        text-gray-600"
              ></div>
                ...
                {Loder === true ? (
                  ...
                  <p
                    className={`text-xs flex items-center gap-2 cursor-pointer hover:bg-gray-200 py-1.5 px-2
                                   ${
                                     workSpaceId._id === e._id && "bg-gray-300"
                                   } group`}
                    onClick={() => {
                      handelSelectedWorkspace(e);
                    }}
                  >
                    <BiGroup className="text-lg text-gray-500" />
                    {e.name}
                    <MdDelete
                      className="hidden group-hover:block ml-auto text-lg hover:text-red-600
                                  text-gray-600"
                    />
                  </p>
                )}
